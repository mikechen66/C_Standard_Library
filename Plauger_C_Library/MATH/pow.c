...
double (pow)(double x, double y) {   // compute x^y
    ...
    if (0 <= errx || 0 < erry) {     // x == 0, INF, NAN; y == INF, NAN
        z = _Nan._D;
        ...
        } else if (errx == INF) {    // INF^finite (NB: erry tests y fraction)
            errx = y < 0.0 ? 0 : INF;
    ...
    if (y <= -dhuge) {
        ...
        zexp = xexp < 0 ? -shuge : xexp == 0 ? 0 : shuge;
    } else {                         // y*log2(x) may be reasonable
