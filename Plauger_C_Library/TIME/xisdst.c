...
int _Isdst(const struct tm *t) {  // test whether Daylight Savings Time in effect

    Dstrule *pr;
    static const char *olddst = NULL;
    static Dstrule *rules = NULL;

    if (olddst != _Times._Isdst) {  // find current dst_rules
        if (_Times._Isdst[0] == '\0') { // look beyond time_zone info
            int n;
    ...
    for (pr = rules; pr->wday != (unsigned char)-1; ++pr) {
        if (pr->year <= t->tm_year) {  // found early enough year
            int rday = _Daysto(t->tm_year, pr->mon) - d0
                 + pr->day;

            if (0 < pr->wday) { // shift to specific weekday
