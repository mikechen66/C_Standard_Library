...
static char *getval(char *s, int val, int n) {  // convert a decimal value
    if (val < 0) {
        val = 0;
    }
...
static int wkyr(int wstart, int wday, int yday) { // find week of year
    wday = (wday + 7 - wstart) % 7;
    return (yday + 7 - wday) / 7;
}

const char *_Gentime(const struct tm *t, _Tinfo *tin,
    char code, int *pn, char *ac) {   // format a time field
    const char *p;
