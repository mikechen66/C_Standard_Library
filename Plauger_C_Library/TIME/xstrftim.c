...
size_t _Strftime(char *buf, size_t bufsize, const char *fmt,
    const struct tm *t, _Tinfo *tin) {  // format time information
    const char *fmtsav, *s;
    size_t len, lensav, nput;
    size_t nchar = 0;

    for (s = fmt, len = strlen(fmt), fmtsav = NULL; ; fmt = s) {  // parse format string
        int n;
        wchar_t wc;
        _Mbsave state = {0};

        while (0 < (n = _Mbtowc(&wc, s, len, &state))) {  // scan for '%' or '\0'
            s += n, len -= n;
            if (wc == '%') {
                break;
            }
        ...
        if (0 == len && fmtsav == NULL || n < 0) {  // format end or bad multibyte char
