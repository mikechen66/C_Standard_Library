...
time_t _Tzoff(void) {  // determine local time offset

    static const char *oldzone = NULL;
    static long tzoff = 0;
    static const long maxtz = 60*13;

    if (oldzone != _Times._Tzone) { // determine time zone offset (East is +)
        const char *p, *pe;
        int n;
...
struct tm *(localtime)(const time_t *tod) { // convert to local time structure
