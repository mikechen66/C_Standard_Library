 ...
 **
 ** _IOFBF: I/O Full Buffer
 ** _IOLBF: I/O Line Buffer
 ** _IONBF: I/O No Buffer
 ** EOF: End of File
 ** L_tmpnam: Long double temporary name
 ** TMP_MAX: Temporary Maximum
 ** SEEK_CUR: Seek Current
 ** fpos_t: file position with type t
 ** _Rend: Read end
 ** _Wend: Write end 
 ** _Cbuf: Character buff
 ** _Nback: Number of backspace 
 ** cleareer: clear error
 ** feof: file end of file
 ** fgetc: file get character
 ** fputc: file put character
 ** fsetpos: file set position
 ** puts: put stream
 ** rewind: get the first character
 ** setbuf: set buff, being equal to setvbuf except no return value
 ** setvbuf: set vertical buff with modes of _IOFBF, IOLBF and IONBF
 ** fprintf: file print format
 ** sprintf: set print format
 ** vfprintf: it equal to fprintf except arg replacing variable parameter
 ** vprintf: it is equal to printf except arg replacing variable parameter
