...
const char *_Skip(const char *s) {   // skip next char plus white-space
    return (*s == '\0' ? s : s + 1 + strspn(s + 1, " \t"));
}

_Linfo *_Getloc(const char *nmcat, const char *lname) {  // get locale pointer, given category and name
    ...
    for (ns = NULL, s = lname; ; s += n + 1) {   // look for exact match or LC_ALL
        if (s[n = strcspn(s, ":;")] == '\0' || s[n] == ';') { // memorize first LC_ALL
            if (ns == NULL) {
                ns = s, nl = n;
            }
            if (s[n] == '\0') {
        ...
        } else if (memcmp(nmcat, s, ++n) == 0) { // found exact category match
    ...
    if (q == NULL) {
        ...
        *p = _Clocale;
        ...
        } else {                                  // parsing error reading locale file
            fputs(buf, stderr);
