...
char *_Fmtval(char *buf, double d, int fdarg) {  // format number by locale-specific rules
    ...
    if (fdarg == FN_INT_CUR) {                  // get international currency parameters
    ...
    } else if (fdarg == FN_LCL_CUR) {           // get local currency parameters
        ...
        grp_sep = p->mon_thousands_sep[0];
        sign = neg ? p->negative_sign : p->positive_sign;
    } else {                                    // get numeric parameters (cur_sym not used)
    ...
    for (s = buf; *fmt; ++fmt, s += strlen(s)) {
        switch (*fmt) {                         // process a format char
            ...
            case 'V':                           // insert formatted value
                ...
                for (ns = 0, i = end - s, g = grps; 0 < i; ++ns) { // count separators to add
