...
char *(setlocale)(int cat, const char *lname) {   // set new locale
    ...
    if (cat != LC_ALL) {                          // set a single category
        ...
        for (i = 0; ++i < _NCAT; ) {             // set a category
            if ((p = _Getloc(nmcats[i], lname)) == NULL) {   // revert all on any failure
    ...
    if (changed) {                               // rebuild curname
        ...
        for (i = 0, n = 0; ++i < _NCAT; ) {
            if (pcats[i] != pcats[0]) {          // count a changed subcategory
                len += strlen(nmcats[i])
                    + strlen(pcats[i]->_Name) + 1;
                ++n;
            }
        }
        if (n == 0) {                            // uniform locale
            if (namalloc) {
                free(curname);
            }
            curname = (char *)pcats[1]->_Name, namalloc = 0;
        } else if ((s = (char *)malloc(len + 1)) == NULL) {   // may be rash to try to roll back 
            setlocale(LC_ALL, curname);
            return (NULL);
        } else {                                 // build complex name
            if (namalloc) {
                free(curname);
            }
            ...
            for (i = 0; ++i < _NCAT; ) {
                if (pcats[i] != pcats[0]) {      // add a component
                    *s++ = ';';
